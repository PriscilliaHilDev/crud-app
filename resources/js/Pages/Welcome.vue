                    ...
                    <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
                        <div class="relative flex h-16 items-center justify-between">
                            ...
                            <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                                <a v-if="$page.props.auth?.user" :href="route('post.list')">
                                ...
                                <div class="hidden sm:ml-6 sm:block">
                                    <div class="flex space-x-4">
                                        <a v-for="item in navigation" :key="item.name" 
                                           :href="item.name === 'Connexion' ? route('login') : (item.name === 'Inscription' ? route('register') : '#')" 
                                           :class="[item.current ? 'bg-pink-800 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium']" 
                    ...
                    <DisclosurePanel class="sm:hidden">
                        <div class="space-y-1 px-2 pb-3 pt-2">
                            <DisclosureButton v-for="item in navigation" :key="item.name" as="a" 
                                              :href="item.name === 'Connexion' ? route('login') : (item.name === 'Inscription' ? route('register') : '#')"  
                                              :class="[item.current ?  'text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium']" 
            ...
            <div class="mx-auto max-w-7xl sm:px-6 sm:py-32 lg:px-8">
                <div class="relative bg-pink-900 px-6 lg:flex lg:rounded-2xl h-[700px]">
                    <div class="mx-auto max-w-md lg:mx-5 flex items-center lg:py-40 lg:text-left">
                        <h2 class="font-bold tracking-tight text-white lg:text-5xl text-3xl lg:py-10 pt-10 text-center">Écrivez et partagez vos plus belles pensées avec nous.</h2>
                    </div>
