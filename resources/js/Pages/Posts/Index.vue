...
<template>
  ...
  <div class="grid container mx-auto grid-cols-1 sm:grid-cols-2 gap-4 max-w-7xl px-6 lg:px-8 xl:grid xl:grid-cols-2 xl:gap-x-10 xl:gap-y-4 ">
    ...
    >
        ...
        <div class="container mx-auto">
          <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
            ...
            <div class="flex justify-end">
              <Link class="-m-1.5 p-1.5" :href="route('post.show', { post: postItem.post })">
              ...
              <div class="mt-6 flow-root">
                <div class="-my-6 divide-y divide-gray-500/10">
                  ...
                  <div class="flex justify-center flex-col items-center">
                    <p class="py-2"> Auteur : {{ postItem.author.name }}</p>
                    <p class="py-2"> Publié le {{ formatDate(postItem.post.created_at) }}</p>
                  </div>
                </div>
              </div>
            </DialogPanel>
      ...
      <img :src="'/storage/' + postItem.image" alt="image de l'article"
      ...
      >
        <div class="mx-auto max-w-2xl">
          <h2 class="font-bold tracking-tight text-slate-600 text-2xl sm:text-2xl lg:text-3xl xl:text-4xl">
            {{ truncateByWords(postItem.post.title, 7) }}
          </h2>
          <p class="mt-6 sm:text-md lg:text-lg xl:text-xl leading-6 sm:leading-7 lg:leading-8 xl:leading-9 text-slate-600">
            {{ truncateByWords(postItem.post.content, 18) }}
          </p>
        </div>
      </div>
      <div class="absolute bottom-0 right-2 z-30 p-4 rounded-lg text-right hidden lg:block">
        <p class="py-2 font-bold text-slate-800 text-md">Auteur : {{ postItem.author.name }}</p>
        <p class="py-2 text-slate-800 text-sm">Publié le {{ formatDate(postItem.post.created_at) }}</p>
      </div>
    </div>
  </div>
